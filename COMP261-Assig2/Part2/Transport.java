...
public class Transport {
    ...
    private static final Set<String> TRAINS = Set.of("JVL_0", "JVL_1", "KPL_0", "KPL_1", "MEL_0", "MEL_1", "HVL_0",
            "HVL_1", "WRL_0", "WRL_1");
    ...
    public static String transpType(String lineID) {
        if (isTrain(lineID)) {
            return TRAIN;
        } else if (isCableCar(lineID)) {
            return CABLECAR;
        } else if (isFerry(lineID)) {
            return FERRY;
        } else if (isWalking(lineID)) {
            return WALKING;
        } else if (isTransfer(lineID)) {
            return TRANSFER;
        } else {
            return BUS;
        }
