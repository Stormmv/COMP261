...
/**
 * Graph is the data structure that stores the collection of stops, lines and
 * connections.
 ...
 * The Lines in the map have their id, and lists of the stopIDs an times (in
 * order)
 *
...
public class Graph {
     ...
     * Remove any stops that are not on any lines since they cannot be accessed from
     * anywhere.
     */
    ...
    public void recomputeWalkingEdges(double walkingDistance) {
        for (Stop from : stops) {
            for (Stop to : stops) {
                if (from != to && from.distanceTo(to) <= walkingDistance) {
                    Edge edge = new Edge(from, to, "walk", null, from.distanceTo(to));
                    edges.add(edge);
                    from.addEdgeOut(edge);
                    to.addEdgeIn(edge);
                }
    ...
    public void removeWalkingEdges() {
        ...
        for (Edge edge : edges) {
            if (edge.transpType().equals("walk")) {
                toRemove.add(edge);
            }
        }
